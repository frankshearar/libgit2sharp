...
namespace LibGit2Sharp
{
    ...
    public class BlameHunk : IEquatable<BlameHunk>
    {
        ...
        internal BlameHunk(IRepository repository, GitBlameHunk rawHunk)
        {
            ...
            {
                FinalSignature = new Signature(Proxy.Std.git_signature_dup(rawHunk.FinalSignature));
            }
            if (rawHunk.OrigSignature != IntPtr.Zero)
            {
                InitialSignature = new Signature(Proxy.Std.git_signature_dup(rawHunk.OrigSignature));
            }
        }
